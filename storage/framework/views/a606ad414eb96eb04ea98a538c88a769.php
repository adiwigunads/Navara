    ...
    <div class="flex min-h-screen">
        <aside class="fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-navara-100 bg-gradient-to-b from-navara-500 to-navara-700 text-white shadow-xl lg:static">
            <div class="flex h-20 items-center gap-3 border-b border-white/10 px-6">
                <img src="<?php echo e(asset('images/navara-logo.png')); ?>" alt="Navara" class="h-11 w-11 rounded-full object-cover ring-2 ring-white/30">
                <div>
            ...
            <nav class="flex-1 space-y-1 p-4 pt-2">
                ...
                <?php $__currentLoopData = $menus; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $menu): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php $active = collect($menu['match'])->contains(fn ($p) => request()->routeIs($p)); ?>
                    <a href="<?php echo e(route($menu['route'])); ?>" class="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition <?php echo e($active ? 'bg-white text-navara-700 shadow-md' : 'text-navara-100 hover:bg-white/10 hover:text-white'); ?>">
                        <svg class="h-5 w-5 shrink-0 <?php echo e($active ? 'text-navara-500' : 'opacity-80'); ?>" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <?php if (! empty(trim($__env->yieldContent('breadcrumb')))): ?>
                            <div class="mb-1 text-xs font-medium text-navara-400"><?php echo $__env->yieldContent('breadcrumb'); ?></div>
                        <?php endif; ?>
                        <h1 class="text-xl font-black text-navara-800"><?php echo $__env->yieldContent('heading', 'Dashboard Verifikator'); ?></h1>
                        <p class="mt-0.5 text-sm text-navara-500"><?php echo $__env->yieldContent('subheading'); ?></p>
                    </div>
                    <div class="flex items-center gap-4">
                        <a href="<?php echo e(route('wisata.index')); ?>" target="_blank" class="rounded-full border border-navara-200 px-4 py-1.5 text-xs font-semibold text-navara-600 hover:bg-navara-50">Lihat Situs</a>
                        <div class="rounded-2xl bg-navara-50 px-4 py-2 text-right">
                            <p class="text-sm font-bold text-navara-800"><?php echo e(auth()->user()->name); ?></p>
...
<?php /**PATH C:\laragon\www\navara\resources\views/layouts/verifikator.blade.php ENDPATH**/ ?>